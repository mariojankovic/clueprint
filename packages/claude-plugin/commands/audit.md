---
description: Get page diagnostics including console errors, network failures, and performance metrics
allowed-tools: mcp__clueprint__audit
argument-hint: [task]
---

Call `mcp__clueprint__audit` to get diagnostics for the current page in the browser.

This returns console errors, network failures, performance metrics (LCP, CLS), and accessibility issues.

If the user provided a task, work on fixing the issues. Otherwise, summarize the problems found.

$ARGUMENTS
